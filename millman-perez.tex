...
by describing what kinds of software tools we need for scientific research.\footnote{We
take it as a for gone conclusion that if we are going to be sharing our research
code with one another, they must be written using open source tools as far
as practical. For the interested reader, we direct them to ``Open source mathematical software'' for a more sustained argument regarding this point \cite{joyner2007open}.}
And then in the following section (§~\ref{subsec:community}) argue that the
scientific community must not only use the right software, but needs to take an
active role in the creation and maintainence of this software as part of our
heritage.
...
Python is open source general purpose programming language initially written
for teaching. It has a simple, expressive, and accessible syntax that
emphasizes code readability (see §~\ref{subsec:readability}). Rather than
imposing a single programming paradigm, it allows one to code at
...
for almost every platform. It is therefore ideal for a heterogeneous
computing environment. Python is also powerful enough to manage the complexity
of large applications, supporting functional programming, object orienting
programming, generic programming, and metaprogramming.  Python has excellent
tools to provide scripting support to tools in other languages including C,
C++, Fortran, and R. This is crucial, both for development and package
integration.  For this reason, Python is often used as a high-level language
glue for calling routines in a huge array of high-quality scientific libraries.
...
\cite{turk2013scale},  \cite{prlic2012ten}
...
We suggest researchers adopt a practice of pervasive version control: research
...
\blockpar{Execution automation: \texttt{make} and friends.}  In addition to
pervasive version control, the most basic practice you \emph{must} adopt is to
automate the execution of as much of your process as you can.  The venerable
\texttt{make} system is the workhorse of process automation for software
compilation and building any outcomes that depend on a chain of inputs.  It has
a syntax for expressing dependencies between sources and targets and a simple
...
our scope.  But whether it's running a sequence of scripts to produce some
figures, compiling your software, or creating the final PDFs for a grant
proposal, you should \emph{always} be able to do so by typing \texttt{make
  results} or the equivalent syntax in your system of choice.  Once things are
automated in this way, it becomes possible for others (humans or machines, and
even yourself on a new system or months down the road) to reliably repeat the
process.  This is therefore a cornerstone of reproducible practices.
...
for having a machine do all the testing for you, all the time, bugging you only
when something goes wrong.  This is known in the software world as
\emph{continuous integration} (CI for short). CI systems are servers that grab
...
log and summarize these results, and to only produce alerts (typically by
email, but more aggressive options such as real-time SMS are possible) when
something goes wrong.  The amount of data collected during the test execution
can be configured, so it
