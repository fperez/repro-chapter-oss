...
To illustrate what we mean, we briefly describe the scientific Python
...
the properties that make it such a useful tool. We consider the scientific Python
ecosystem as a case study for the type of community developed software stack that
we believe can lead to reproducible computational results.
...
finding wide adoption across research disciplines, education, and industry as well as
a core part of the computational infrastructure at institutions such as CERN and the Hubble
...
Fortran, and R. As such, Python is often used as a high-level language glue for calling
...
Rather, scientists have extended it with additional libraries that add features necessary for scientific work.
...
While the case can be made for the use of open source software in science,
even more importantly is the benefit that comes with open source community-driven
development practices. In particular, our computational tools must move
...
idealy with a meaningful \emph{commit message}.  The
...
instructions again later on, but a much more sensible approach is
...
in the development process.  For program that accept input either from a user
or file, it is important that the code checks that the input is exactly what
the program expects. Test that ensure individual code elements like functions,
...
effort to develop a documentation string standard.\footnote{cite online doc editor} In Python, a documentation
...
get out of sync with the actual code. \fix{benefits of standard include: causes
authors to explicitly think about input, output, references. makes them more rigorous
in coding. otherwise when you look for the paper years later and have no idea where to go}
