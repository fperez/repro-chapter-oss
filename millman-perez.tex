...
In December of 2004, we were both at a meeting on improving the software used
in neuroimaging analysis. It was the first time we met: one of us (JM) was in
charge of the data analysis system at Berkeley's Brain Imaging Center (BIC) and
the other (FP) was a physics graduate student. We quickly realized that we had
the same perspective on the importance of good computational tools and practice
to the progress of scientific research.  We also shared the concern that most
researchers did not have good training in computation, scientific coding, or
data management. And we saw that this made it hard to document and replicate
analyses.

As an immediate result of that meeting, we co-wrote one of the individual
research plans of a much larger program project proposal titled ``Building a
platform for analysis replication and code sharing in neuroimaging''. While
the grant wasn't funded, the meeting led to the Neuroimaging in Python (NIPY)
project. It also led indirectly to FP joining the BIC as a research scientist
in \fix{2008}.
