...
Whether you are collecting data, running different analyses,
or writing papers, you will inevitably need to keep track
of the various versions of your work:  data is augmented and curated;
code is adapted and improved; and writing is revised and expanded.
While simply keeping the most recent version of your work sounds
like a reasonable approach, in practice, this is seldom sufficient.
...
partial solutions to the problem, they are also cumbersome and prone to failure.

Since tracking and managing how work evolves over time, programmers have
developed specialized software tools to do exactly this. These software tools
are called \emph{version control} systems. Several open source version
control systems (VC) are available including CVS, SVN, Git, and Mecurial. Graphical
interfaces exist for many of these tools, but our prefered mechanism for interaction
is the commandline.
...
you want to track, you can \emph{commit} those changes with a \emph{commit message}.
These messages are very important if you need to revert to a different commit.
...
your commits will resemble a tree with several \emph{branches} diverging from a common
base or trunk. When exploring these alternative approaches on different branches,
you may find that several branches naturally start converging. At this point, you
will need a way to \emph{merge} these branches back together. If you worked on
different parts of your project in the separate branches, merging those parts
together is straightforward and completely automatic. When you have \emph{conflicting}
changes in different branches (e.g., edits to the same line of code), then the
VC systems require manual intervention. While any VC system
must allow commits in a repository, their support for efficient branching and
merging vary greatly.
...
the nature of the bug, it may reveal a fundamental problem with the overall
...
To assist in this process, programmers have developed a number of tools and practices
...
literate programming \cite{knuth_literate} and mention current progress in that
...
If we want to ensure that any computational research is truly reproducible, we
need to consider this entire life-cycle in an integral way and from the
beginning.  Asking about reproducibility by the time a manuscript is ready for
submission to a journal is simply too late: this problem must be tackled from
the start as one of tools and practices throughout the process of research, not
as an afterthought tacked-on at publication time.  Furthermore, it must be
considered in an integral way, so that we can move fluidly \emph{back and
  forth} between the above steps. The pressure to publish in today's world
encourages all scientists to charge forward chasing the goal of an accepted
manuscript, but the very term ``reproducibility'' implies repetition and thus a
requirement to also move \emph{back} to retrace one's steps, question or change
assumptions and move forward again.  Therefore, we should look for approaches
that allow the researcher to move back and forth as the problem dictates rather
than funneling him or her blindly towards a published result only.
...
into these documents and will therefore diverge XXX

so that the same content is duplicated in many different
places. When the artifacts change, the documents become out of sync and require
